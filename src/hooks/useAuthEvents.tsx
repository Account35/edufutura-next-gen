  ...
  useEffect(() => {
    const { data: { subscription } } = supabase.auth.onAuthStateChange(
      (event, session) => {
        ...
        // Keep the auth callback synchronous to avoid blocking sign-in/sign-out state updates.
        setTimeout(() => {
          ...
          if (isNewUser && !hasTriggeredWelcome.current.has(userId)) {
            ...
            void supabase.functions.invoke('welcome-new-user', {
          ...
          if (event === 'SIGNED_IN' && !hasTriggeredLogin.current.has(userId)) {
            ...
            void supabase.functions.invoke('track-login', {
              body: { user_id: userId }
            }).then(({ data }) => {
