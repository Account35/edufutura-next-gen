...
const fetchAdminStats = async (): Promise<AdminStatsData> => {
  const now = new Date();
  ...
  // Get daily active users for last 7 days (parallelized)
  ...
  return { stats, engagement, health };
};
...
export const useAdminStats = () => {
  ...
  return {
    ...
    engagement: data?.engagement ?? defaultEngagement,
    health: error ? { ...defaultHealth, databaseConnected: false } : (data?.health ?? defaultHealth),
    loading: isLoading,
    refetch,
  };
};
